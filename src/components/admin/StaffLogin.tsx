...
import { claimOwnership, verifyLoginCode } from "@/lib/admin.functions";

type Props = {
  /** Secret per-staff code from the URL. When omitted this is the owner bootstrap page. */
  code?: string;
  /** Path to return to after email confirmation. */
  redirectPath: string;
};

export function StaffLogin({ code, redirectPath }: Props) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (code) {
        const result = await verifyLoginCode({ data: { code } }).catch(() => ({ ok: false }));
        if (!result.ok) {
          await supabase.auth.signOut();
          throw new Error("This private link does not belong to your account.");
        }
      }
    ...
    <div className="flex min-h-screen flex-col bg-surface">
      ...
      <main className="mx-auto flex w-full max-w-md flex-1 flex-col justify-center px-4 py-10">
        ...
        <p className="mt-4 text-center text-[11px] text-muted-foreground">
          {code
            ? "This private link works only for the account it was issued to."
